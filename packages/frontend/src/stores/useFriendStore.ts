...
export const useFriendStore = create<FriendState>((set, get) => ({
  ...
  addFriend: async (to, message) => {
    try {
      ...
      // Reload friend requests to update sentList
  ...
  acceptRequest: async (requestId) => {
    try {
      ...
      // Lấy fromUserId trước khi xóa khỏi list
      ...
      // Reload friend list after accepting
      await get().getFriends();

      // Tự động tạo cuộc trò chuyện 1-1 với người vừa kết bạn
      if (fromUserId) {
        await useChatStore.getState().createDirectConversation(fromUserId);
      }
    } catch (error) {
