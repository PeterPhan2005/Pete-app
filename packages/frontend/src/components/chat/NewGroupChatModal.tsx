...
const NewGroupChatModal = () => {
  ...
  const handleSelectFriend = (friendData: {
    ...
    setInvitedUsers([...invitedUsers, friendData]);
    setSearch("");
  };

  const handleRemoveFriend = (friendData: {
    _id: string;
    username: string;
    displayName: string;
    avatarUrl?: string;
  }) => {
    setInvitedUsers(invitedUsers.filter((u) => u._id !== friendData._id));
  };
  ...
  const filteredFriends = friends
    .map(friend => friend.friendId) // Extract friendId from Friend object
    .filter(friendData => 
      friendData?.displayName?.toLowerCase().includes(search.toLowerCase()) &&
      !invitedUsers.some((u) => u._id === friendData._id)
    );
